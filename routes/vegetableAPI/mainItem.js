...
router.get('', (req, res) => {
    ...
    MongoClient.connect(process.env.mongoDBConnector, {useNewUrlParser: true ,useUnifiedTopology: true}).then(function(db) {
        db.db('Vegetable').collection('MainPage').find({}, 
        ).toArray(async(err, result) => {
            if (err) throw res.status(400).send('Error')
            for (i =0; i< result[0]['recommend'].length; i++){
                var fruit = await  db.db('Vegetable').collection('allVegetable').findOne({'_id':ObjectId(result[0]['recommend'][i])},
                {projection : {
            ...
            for (i =0; i< result[0]['hotsale'].length; i++){
                var fruit = await  db.db('Vegetable').collection('allVegetable').findOne({'_id':ObjectId(result[0]['hotsale'][i])},
                {projection : {
                    _id : 1, name:1, price:1
                    }
                });
